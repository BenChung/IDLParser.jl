# The canonical generated interface types (static), defined first so every
# component file can reference `Interfaces.<pkg>.<qual>.<Name>` — the wire types
# the §11/§13 bootstrap, parameters, lifecycle, actions, and /rosout all marshal.
#
# `@ros_msgs` over the *whole* vendored tree emits the nested
...
# Self-contained (only `@ros_msgs`), hence include-first.
#
...
# `canonical_type` in wellknown.jl). The `type_description_interfaces` subtree
# additionally backs the §11/§13 bootstrap — the wire⇄internal bridge in
