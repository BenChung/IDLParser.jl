...
resolve_constants(definition::Parse.Annotated.Type, local_scope::Dict{Symbol, Any}, file_scope::Dict{Symbol, Any}) = @match definition begin 
        Parse.Annotated.Annotation(name, params, defn) => begin 
            new_params = [name => resolve_constants(value, local_scope, file_scope) for (name, value) in params]
            return Annotated.Annotation(name, new_params, resolve_constants(defn, local_scope, file_scope))
        end
    end
...
resolve_constants(definition::Parse.TypeSpec.Type, local_scope::Dict{Symbol, Any}, file_scope::Dict{Symbol, Any}) = @match definition begin 
    Parse.TypeSpec.TRef(name) => TypeSpec.TRef(name)
...
resolve_constants((ts, decls)::Pair{TypeSpec.Type, Vector{Declarator.Type}}, local_scope, file_scope) = 
...
resolve_constants(decl::Parse.Declarator.Type, local_scope, file_scope) = @match decl begin 
    Parse.Declarator.DArray(name, dims) => Declarator.DArray(name, dims)
...
resolve_constants(definition::Parse.TypeDecl.Type, local_scope::Dict{Symbol, Any}, file_scope::Dict{Symbol, Any}) = @match definition begin 
    Parse.TypeDecl.StructDecl(name, super, decls) => TypeDecl.StructDecl(name, super, let ls = Dict{Symbol, Any}(); [resolve_constants(decl, ls, file_scope)] end)
    ...
    Parse.TypeDecl.TypedefDecl(def, decls) => TypeDecl.TypedefDecl(resolve_constants(def, local_scope, file_scope), [resolve_constants(decls, local_scope, file_scope) for decl in decls])
